    ...
    return (
        <div className="max-w-2xl mx-auto space-y-8">
                ...
                <div className="glass-panel p-6 rounded-2xl bg-white border border-slate-200">
                    <form onSubmit={handleGenerate} className="flex gap-4">
                        <input
                            ...
                            className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:ring-2 focus:ring-purple-500/50 shadow-sm placeholder-slate-400"
                        />
                        <button type="submit" disabled={!topic} className="btn-primary from-purple-500 to-violet-500">
                            Generate
                        </button>
                    </form>
                </div>
            ...
            {quiz && !showResult && !loading && (
                <div className="glass-card p-8 rounded-2xl border-purple-200 bg-white">
                    <div className="mb-6 flex justify-between items-center text-sm text-slate-500">
                        <span>Question {currentQuestion + 1} of {quiz.questions.length}</span>
                        <span className="text-purple-600 font-medium">{quiz.title}</span>
                    </div>
                    ...
                    <div className="space-y-3">
                        {quiz.questions[currentQuestion].options.map((option: string, idx: number) => {
                            ...
                            let style = "bg-white border-slate-200 hover:bg-slate-50 text-slate-700"
                            if (isAnswered) {
                                if (isSelected && !isCorrect) style = "bg-purple-50 border-purple-200 text-purple-700"
                    ...
                    {isAnswered && (
                        <div className="mt-8 flex justify-end">
                            <button onClick={nextQuestion} className="btn-primary from-purple-500 to-violet-500">
            ...
            {showResult && (
                ...
                >
                    <Trophy className="mx-auto text-purple-500 mb-6" size={64} />
                    <h2 className="text-3xl font-bold text-slate-900 mb-2">Quiz Completed!</h2>
                    <p className="text-slate-500 mb-6">You scored</p>
                    <div className="text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-violet-600 mb-8">
