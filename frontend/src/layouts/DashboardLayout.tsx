...
const SidebarItem = ({ icon: Icon, label, to, active }: any) => (
    ...
    >
        <Icon size={20} className={clsx("transition-transform group-hover:scale-110", active ? "text-violet-500" : "text-slate-400 group-hover:text-violet-500")} />
        <span>{label}</span>
    </Link>
)
...
export default function DashboardLayout() {
    ...
    return (
        <div className="flex h-screen bg-slate-50 overflow-hidden selection:bg-violet-100 selection:text-violet-900">
            {/* Sidebar */}
            <aside className="w-72 bg-white border-r border-slate-200 p-6 flex flex-col hidden md:flex z-20 shadow-sm">
                <div className="flex items-center gap-3 mb-10 px-2">
                    <div className="p-2 bg-gradient-to-br from-violet-400 to-purple-500 rounded-lg shadow-lg shadow-purple-500/20">
                        <Brain className="text-white" size={24} />
                    </div>
                    <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-600 to-purple-500">
                        StudyBuddy
                    </span>
                </div>
            ...
            <main className="flex-1 relative overflow-hidden flex flex-col">
                ...
                <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0 opacity-40">
                    <div className="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] rounded-full bg-violet-200/20 blur-[100px]" />
                    <div className="absolute top-[20%] right-[10%] w-[30%] h-[30%] rounded-full bg-purple-200/20 blur-[80px]" />
                </div>
